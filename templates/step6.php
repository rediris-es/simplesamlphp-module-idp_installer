...
?>
<p><?php echo $this->t('{sir_install:sir_install:step6_cert_select}'); ?>
...
$inputs_create = '<h4>'.$this->t('{sir_install:sir_install:step6_create_title}').'</h4>';
$inputs_create.= "<p style='margin-top:3px;'>" . $this->t('{sir_install:sir_install:step6_cert_info}');
$inputs_create.= '<input type="text" name="organization_name" value="" style="width: 500px; margin-top:5px;"/></p>';

$inputs_import = '<h4>'.$this->t('{sir_install:sir_install:step6_import_title}').'</h4>';
$inputs_import.= "<p style='margin-top:3px;'>". $this->t('{sir_install:sir_install:step6_cert_info2}').'</p>';
...
<div id = "create_form" <?php if ($selected == "import") { ?>style="display:none;" <?php } ?>>
    <?php
    drawButton($next_step, $this->t('{sir_install:sir_install:next_step}'), '', $inputs_create);
    ?>
</div>
<div id = "import_form" <?php if ($selected == "create") { ?>style="display:none;" <?php } ?>>
    <?php
    drawButton($next_step, $this->t('{sir_install:sir_install:next_step}'), '', $inputs_import);
    ?>
</div>
<?php
$dir_certs = realpath(__DIR__ . "/../../../") . "/cert";
echo "<p style='margin-top:3px;'>" . $this->t('{sir_install:sir_install:step6_footer_note}') . '<i>' . $dir_certs . '</i></p>';
?>