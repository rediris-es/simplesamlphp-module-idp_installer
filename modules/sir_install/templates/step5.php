...
//PASO 5 del instalador.
    ...
    ?>
    <p><?php echo $this->t('{sir_install:sir_install:step5_datasource_select}'); ?>
    ...
    <div onload = "showDatasource();">
        <div id = "ldap_form" <?php if ((!$ldap && $pdo)||$selected=="pdo") { ?>style="display:none" <?php } ?>>
            <h3><?php echo $this->t('{sir_install:sir_install:step5_ldap_title}'); ?></h3>
            <p>
                <?php echo $this->t('{sir_install:sir_install:step5_ldap_hostname}'); ?><br/>
                <input type="text" name="ldap_hostname" value="" style="width: 300px;"/><br/>
                <?php echo $this->t('{sir_install:sir_install:step5_ldap_example}'); ?><br/>
            </p>
            <p>   
                <?php echo $this->t('{sir_install:sir_install:step5_ldap_enable}'); ?><br/>
                <select name="ldap_enable_tls">
                    <option value="0"><?php echo $this->t('{sir_install:sir_install:step5_yes}'); ?></option>
                    <option value="1"><?php echo $this->t('{sir_install:sir_install:step5_no}'); ?></option>
                </select><br/>
            </p>
            <p>    
                <?php echo $this->t('{sir_install:sir_install:step5_ldap_referral}'); ?><br/>
            ...
            <div class="caution" style="min-height:45px;">
                <?php echo $this->t('{sir_install:sir_install:step5_ldap_info}'); ?>
            </div>
        </div>
        <div id="pdo_form" <?php if ((!$pdo && $ldap) || ($ldap && $pdo && $selected=="ldap")) { ?>style="display:none" <?php } ?>>
            <h3><?php echo $this->t('{sir_install:sir_install:step5_pdo_title}'); ?></h3>
            <p>
                <?php echo $this->t('{sir_install:sir_install:step5_pdo_dsn}'); ?><br/>
                <input type="text" name="pdo_dsn" value="" style="width: 600px;"/><br/>
                <?php echo $this->t('{sir_install:sir_install:step5_pdo_example}'); ?><br/>
            </p>
            <p>
                <?php echo $this->t('{sir_install:sir_install:step5_pdo_username}'); ?><br/>
                <input type="text" name="pdo_username" value="" style="width: 300px;"/><br/>
            </p>
            <p>
                <?php echo $this->t('{sir_install:sir_install:step5_pdo_password}'); ?><br/>
