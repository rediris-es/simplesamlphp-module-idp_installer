 ...
 */
function idp_installer_hook_step7(&$data) {
    ...
    if ($new_certificate || $old_certificate) {
        ...
        if (!file_exists($dir_certs) && !is_writable($dir_certs . "/../")) {
            $data['errors'][] = $data['ssphpobj']->t('{idp_installer:idp_installer:step6_mkdir_cert_error}');
            ...
            $data['errors'][] = $data['ssphpobj']->t('{idp_installer:idp_installer:step6_perm_cert_error}');
        ...
        if ($old_certificate) {
            ...
            if (openssl_x509_check_private_key($cert, $priv_key)) {
                ...
                if ($a1 != null) {
                    ...
                    if (is_array($details) && array_key_exists("type", $details) && $details["type"] == OPENSSL_KEYTYPE_RSA) {
                        ...
                        if ($x === false || $y === false) {
                            $data['errors'][] = $data['ssphpobj']->t('{idp_installer:idp_installer:step6_cert_error}');
                            return true;
                        }
                    } else {
                        $data['errors'][] = $data['ssphpobj']->t('{idp_installer:idp_installer:step6_cert_rsa_error}');
                        return true;
                    }
                } else {
                    $data['errors'][] = $data['ssphpobj']->t('{idp_installer:idp_installer:step6_cert_rsa_error}');
                ...
                $data['errors'][] = $data['ssphpobj']->t('{idp_installer:idp_installer:step6_cert_error_comparation}');
                return true;
            }
        } else {
            ...
            if($outCode !== 0){
                $data['errors'][] = $data['ssphpobj']->t('{idp_installer:idp_installer:step7_command_error}').'</br></br><pre>'.$respStderr.'</pre>';
                }
        }
        ...
        if (!file_exists($crt) || !file_exists($pem)) {
            ...
            $data['errors'][] = $data['ssphpobj']->t('{idp_installer:idp_installer:step6_cert_error}');
            if (isset($command)) {
                $data['errors'][] = $data['ssphpobj']->t('{idp_installer:idp_installer:step6_cert_error_suggest}');
                $data['errors'][] = $command;
                return true;
            }
        }
        ...
        $data['errors'][] = $data['ssphpobj']->t('{idp_installer:idp_installer:step6_org_name_empty_error}');
        return true;
    }
    ...
    if (!$res || count($perms_ko) > 0) {
        if (function_exists('posix_getgrnam')) {
            $aux = "<br/>" . $data['ssphpobj']->t('{idp_installer:idp_installer:step7_error}');
            $aux .= "<br/>" . $data['ssphpobj']->t('{idp_installer:idp_installer:step4_perms_ko}');
            $filename = $perms_ko[0];
    ...
    if (count($data['errors2']) == 0) {
        ...
        $data['errors2'][] = $data['ssphpobj']->t('{idp_installer:idp_installer:step7_error}');
    }
    return $res;
}
