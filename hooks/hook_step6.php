 ...
 */
function simplesamlphp-module-idp-installer_hook_step6(&$data) {
    ...
    if (isset($_REQUEST['data_source_type'])) {
        ...
        if (strcmp($ds_type, "ldap") == 0 && ($data['datasources'] == "all" || $data['datasources'] == "ldap")) {
            if (array_key_exists('ldap_hostname', $_REQUEST) && !empty($_REQUEST['ldap_hostname']) && array_key_exists('ldap_enable_tls', $_REQUEST) && array_key_exists('ldap_referral', $_REQUEST)) {
                ...
                $res = @ldap_bind($res);
                if (!$res) {
                    $data['errors'][]            = $data['ssphpobj']->t('{simplesamlphp-module-idp-installer:simplesamlphp-module-idp-installer:step5_datasource_error}');
    ...
    $data['errors'][] = $data['ssphpobj']->t('{simplesamlphp-module-idp-installer:simplesamlphp-module-idp-installer:step5_datasource_request_error}');
    return true;
}
