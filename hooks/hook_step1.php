 ...
 * 
 */
function simplesamlphp-module-idp-installer_hook_step1(&$data) {
    $ssphpobj = $data['ssphpobj'];
    //Comprobamos la versión de PHP
    if (version_compare(PHP_VERSION, "5.3.0", ">=") === false) {
        $data['errors'][] = $ssphpobj->t('{simplesamlphp-module-idp-installer:simplesamlphp-module-idp-installer:general_error}');
        $data['errors'][] = $ssphpobj->t('{simplesamlphp-module-idp-installer:simplesamlphp-module-idp-installer:step1_error_version}');
    } else {
        ...
        if (count($failed_extensions) > 0) {
            $data['errors'][] = $ssphpobj->t('{simplesamlphp-module-idp-installer:simplesamlphp-module-idp-installer:general_error}');
            $data['errors'][] = $ssphpobj->t('{simplesamlphp-module-idp-installer:simplesamlphp-module-idp-installer:step1_error_extensions}') . implode(", ", $failed_extensions) . ".";
        } else {
            //Continuamos comprobando memcache (opcional)
            if (!isExtensionActive('memcached', $loaded_extensions)) {
                $data['warning'][] = $ssphpobj->t('{simplesamlphp-module-idp-installer:simplesamlphp-module-idp-installer:step1_memcached_error}');
            }
            ...
            if ($ldap && $pdo && $modulos) {
                ...
                $data['errors'][] = $ssphpobj->t('{simplesamlphp-module-idp-installer:simplesamlphp-module-idp-installer:general_error}');
                $data['errors'][] = $ssphpobj->t('{simplesamlphp-module-idp-installer:simplesamlphp-module-idp-installer:step1_any_data_source_error}');
            } else if (!$ldap && $pdo && $modulos) {
                //Solo PDO
                $aux               = $ssphpobj->t('{simplesamlphp-module-idp-installer:simplesamlphp-module-idp-installer:step1_only_pdo}');
                $aux.= " " . $ssphpobj->t('{simplesamlphp-module-idp-installer:simplesamlphp-module-idp-installer:step1_only_pdo_modules_list}');
                $aux.= implode(", ", getPDOModulesActive($loaded_extensions)) . ".";
                $data['warning'][] = $aux;
            } else if (!$ldap && $pdo && !$modulos) {
                //Necesita modulos $pdo
                $data['errors'][] = $ssphpobj->t('{simplesamlphp-module-idp-installer:simplesamlphp-module-idp-installer:general_error}');
                $data['errors'][] = $ssphpobj->t('{simplesamlphp-module-idp-installer:simplesamlphp-module-idp-installer:step1_pdo_no_modules}');
            } else if ($ldap) {
                //Solo LDAP ($ldap y me da igual si $pdo o $modulos estan ok, 
                //ya se que algo falla por que no entré en caso 1
                $data['warning'][] = $ssphpobj->t('{simplesamlphp-module-idp-installer:simplesamlphp-module-idp-installer:step1_only_ldap}');
            }
            $files    = array(
                "config/config.php",
            ...
            if (count($perms_ko) > 0) {
                $aux = $ssphpobj->t('{simplesamlphp-module-idp-installer:simplesamlphp-module-idp-installer:step1_perms_ko}');
                $aux.= "<ul style='margin-top:30px;'><li>".implode("</li><li>",$perms_ko)."</li></ul>";
                $aux.= $ssphpobj->t('{simplesamlphp-module-idp-installer:simplesamlphp-module-idp-installer:step1_perms_ko2}');

                if(function_exists('posix_getgrnam')){                    
                    $aux.= "<br/>".$ssphpobj->t('{simplesamlphp-module-idp-installer:simplesamlphp-module-idp-installer:step1_perms_ko3}');
                    $filename = $perms_ko[0];
                ...
                $data['errors'][] = $aux;
                $data['errors'][] = $ssphpobj->t("{simplesamlphp-module-idp-installer:simplesamlphp-module-idp-installer:step1_remember_change_perms}");
            }
        }
    }
    ...
    if (count($data['errors']) == 0) {
        $data['info'][] = $ssphpobj->t('{simplesamlphp-module-idp-installer:simplesamlphp-module-idp-installer:step1_all_ok}');
    }
    return true;
}
