 ...
 */
function sir_install_hook_step8(&$data) {
    ...
    if (curl_exec($ch) === false) {
        $data['errors'][] = $data['ssphpobj']->t('{sir_install:sir_install:step8_curl_error}');
    ...
    if (array_key_exists('saml20-sp-remote', $output) && array_key_exists('saml20-idp-remote', $output)) {
        ...
        $data['errors'][] = $data['ssphpobj']->t('{sir_install:sir_install:step8_curl_error2}');
    }
    if (!$res || count($perms_ko) > 0) {
        if (function_exists('posix_getgrnam')) {
            $aux       = "<br/>" . $data['ssphpobj']->t('{sir_install:sir_install:step8_error}');
        ...
        $data['errors'][] = $aux;
        $data['errors'][] = $data['ssphpobj']->t("{sir_install:sir_install:step1_remember_change_perms}");
    }
    if (count($data['errors']) == 0) {
        $data['info'][] = $data['ssphpobj']->t('{sir_install:sir_install:step8_all_ok}');
    }
