 ...
 */
function sir_install_hook_step5(&$data) {
    $data['datasources'] = getDataSources();
    $require_mods = array("saml", "sir_install", "modinfo", "ldap", "sqlauth", "core", "portal"); //Modulos obligatorios
            ...
            if (in_array($m, $require_mods)) { //PARA LOS QUE SI QUEREMOS ACTIVAR
                if (file_exists($f . '/default-disable')) {
                    ...
                    if (!file_exists($f . '/default-enable')) {
                        $data['errors'][] = $ssphpobj->t('{sir_install:sir_install:step4_error}');
                    }
                }
    ...
    if (count($modules_ko) > 0) {
        ...
        if (function_exists('posix_getgrnam')) {
            $aux = "<br/>" . $ssphpobj->t('{sir_install:sir_install:step4_perms_ko}');
            $filename = $perms_ko[0];
    ...
    if (count($data['errors']) == 0) {
        $data['info'][] = $ssphpobj->t('{sir_install:sir_install:step4_all_ok}');
    } /*else {
        $data['errors'][] = $ssphpobj->t('{sir_install:sir_install:step4_error}');
    }*/
    return true;
